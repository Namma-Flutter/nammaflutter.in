      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
          {teamData.members.map((member, index) => (
            <div
              key={index}
              className="flutter-animated-border p-[2px] rounded-[1.5rem] group relative hover:scale-105 transition-transform duration-300"
            >
              <div className="flex flex-col items-center p-8 bg-white/90 rounded-[1.5rem] shadow-lg group-hover:shadow-flutter-glow border border-[#B3E5FC] w-full">
                <div className="relative mb-4">
                  <img
                    src={member.image}
                    alt={`${member.name}'s profile`}
                    className="w-32 h-32 rounded-full object-cover border-4 border-[#42A5F5] shadow-md group-hover:shadow-lg transition-all"
                  />
                </div>
                <div className="flex justify-center mt-2 mb-2 w-full">
                  <span
                    className="bg-gradient-to-r from-[#42A5F5] to-[#00C6FB] text-white px-2 py-1 rounded-full text-xs font-bold shadow max-w-[150px] truncate cursor-pointer group/role text-center"
                    title={member.role}
                  >
                    {member.role}
                  </span>
                </div>
                <h3 className="text-xl font-bold text-[#02569B] mb-1 text-center">{member.name}</h3>
